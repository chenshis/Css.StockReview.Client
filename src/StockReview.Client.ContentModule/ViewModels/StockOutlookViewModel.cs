...
namespace StockReview.Client.ContentModule.ViewModels
{
    ...
    public class StockOutlookViewModel : NavigationAwareViewModelBase
    {
        ...
        /// 联动数据
        /// </summary>
        public ObservableCollection<string> LinkageData { get; set; } = new ObservableCollection<string>();
        ...
        public override void Refresh()
        {
            ...
            Task.Run(() =>
            {
                try
                {
                    ...
                    if (emotionDetail?.histogram != null)
                    {
                    ...
                    System.Windows.Application.Current.Dispatcher.Invoke(new Action(() =>
                    {
                        ...
                        if (todayLimitUpDetail != null)
                        {
                            LimitUpDetails.Add(todayLimitUpDetail);
                        }
                        if (yesterdayLimitUpDetail != null)
                        {
                    ...
                    else
                    {
                        CandleSeries = null;
                        CandleXAxes = null;
                        CandleVolumeSeries = null;
                        TimeXAxes = null;
                        TimeSeries = null;
                        TimeVolumeSeries = null;
                    }
                }
                catch (Exception ex)
                {
        ...
        /// 获取联动数据
        /// </summary>
        private void GetLinkageData()
        {
            System.Windows.Application.Current.Dispatcher.Invoke(() =>
            {
                LinkageData.Clear();
            });
            Process[] processesByName = Process.GetProcessesByName("TdxW");
            if (processesByName.Length != 0)
            {
                ...
                {
                    string item;
                    if ((!string.IsNullOrEmpty(processesByName[i].MainWindowTitle)))
                    {
                        item = (processesByName[i].MainWindowTitle.Substring(0, 4));
                    }
                    else
                    {
                        item = ("未知");
        ...
        private void RealTimeBulletinBoard()
        {
            ...
            _retryPolicy.Execute(() =>
            {
                ...
                if (apiResponse.Code != 0)
                {
                    ...
                    BulletinBoard = new BulletinBoardDto();
                    return;
                }
        ...
        private void RealTimeSeries(object parameter)
        {
            ...
            if (parameter is LimitUpStockDetailModel model)
            {
                ...
                TimeVolumeSeries = new ISeries[]
                {
                    new ColumnSeries<double>
                    {
                        Values = apiResponse.Data.StockDetailData.Turnovers.ToArray(),
                        Name = "成交量"
                    }
                };
